...
@Controller
@RequestMapping("/reportWeekRemotePolicy")
public class ReportWeekRemotePolicyController {
	private static final Log log = LogFactory.getLog(ReportWeekRemotePolicyController.class);

	@Autowired
	@Qualifier("ReportWeekRemotePolicyService")
	private ReportWeekRemotePolicyService reportWeekRemotePolicyService;

	@RequestMapping("/queryReportWeekRemotePolicy")
	public void queryReportWeekRemotePolicy(HttpServletRequest request, HttpServletResponse response, FormDto dto) throws GeneralException {
		...
		try {
			pageDto = reportWeekRemotePolicyService.findReportWeekRemotePolicyByWhere(pageDto);
	...
	public void queryDataToExcel(HttpServletRequest request, HttpServletResponse response, FormDto dto) throws GeneralException {
		...
		try {
			//不分页查询所有数据
			pageDto = reportWeekRemotePolicyService.queryDataToExcel(pageDto);
			//定义从数据库取出数据顺序数组
			String[] colum_name = {"deptNameTwo", "deptNameThree", "deptNameFour", "chennelName",
					"remoteNodeName", "foundDate","planFee",
					"signLastWeek","signThisWeek","signThisMonth","signThisYear",
					"incomeLastWeek","incomeThisWeek","incomeThisMonth","incomeThisYear",
			...
			String strFileName ="reportWeekRemotePolicyModel.xls";
