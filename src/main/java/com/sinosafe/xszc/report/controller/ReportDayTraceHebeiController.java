...
import com.sinosafe.xszc.report.service.ReportDayTraceHebeiService;
...
@Controller
@RequestMapping("/reportDuty")
public class ReportDayTraceHebeiController {

	private static final Log log = LogFactory.getLog(ReportDayTraceHebeiController.class);
	
	@Autowired
	@Qualifier("ReportDayTraceHebeiService")
	private ReportDayTraceHebeiService reportDayTraceHebeiService;
	 ...
	 */
	@RequestMapping("/queryReportDuty")
	public void queryReportDuty(HttpServletRequest request, HttpServletResponse response, FormDto dto) throws GeneralException {
		...
		try {
			pageDto = reportDayTraceHebeiService.findReportDayTraceHebeiByWhere(pageDto);
		} catch (Exception e) {
			log.error(e);
	...
	public void queryDataToExcel(HttpServletRequest request, HttpServletResponse response, FormDto dto) throws GeneralException {
		...
		try {
			//不分页查询所有数据
			pageDto = reportDayTraceHebeiService.queryDataToExcel(pageDto);

			//定义从数据库取出数据顺序数组
			String[] colum_name = {"updateDate", "vhlType", "primaryCProdName",
					"nPrm", "cInterCde"};
			//模板路径
			...
			String strFileName ="reportDayTraceHebeiModel.xls";
