...
@Controller
@RequestMapping("/reportMonthRemotePolicy")
public class ReportMonthRemotePolicyController {

	private static final Log log = LogFactory.getLog(ReportMonthRemotePolicyController.class);

	@Autowired
	@Qualifier("ReportMonthRemotePolicyService")
	private ReportMonthRemotePolicyService reportMonthRemotePolicyService;

	@RequestMapping("/queryReportMonthRemotePolicy")
	public void queryReportMonthRemotePolicy(HttpServletRequest request, HttpServletResponse response, FormDto dto) throws GeneralException {
		...
		try {
			pageDto = reportMonthRemotePolicyService.findReportMonthRemotePolicyByWhere(pageDto);
	...
	public void queryDataToExcel(HttpServletRequest request, HttpServletResponse response, FormDto dto) throws GeneralException {
		...
		try {
			// 不分页查询所有数据
			pageDto = reportMonthRemotePolicyService.queryDataToExcel(pageDto);
			// 定义从数据库取出数据顺序数组
			String[] colum_name = { "deptNameTwo", "deptNameThree", "deptNameFour", "chennelName", "remoteNodeName", "foundDate", "planFee",
					"incomeLastMonth", "incomeThisMonth", "incomePeriodMonth", "incomeThisYear", "incomePlanPercent", "incomeLastYear",
					"incomeIncreateRate", "incomeYear", "signLastMonth", "signThisMonth", "signThisYear", "claimLastMonth", "claimThisMonth" };
			// 模板路径
			...
			String strFileName = "reportMonthRemotePolicyModel.xls";
