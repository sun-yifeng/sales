...
@Controller
@RequestMapping("/reportFoundationMedium")
public class ReportFoundationMediumController {

	private static final Log log = LogFactory.getLog(ReportFoundationMediumController.class);
	...
	public void queryReportDayAgentChannel(HttpServletRequest request, HttpServletResponse response, FormDto dto) throws GeneralException {
		...
		try {
			pageDto = reportFoundationMediumService.findReportDayAgentChannelByWhere(pageDto);
		} catch (Exception e) {
			log.error(e);
	...
	public void queryDataToExcel(HttpServletRequest request, HttpServletResponse response, FormDto dto){
		String startStr = request.getParameter("start");
		String limitStr = request.getParameter("limit");
		PageDto pageDto = new PageDto();
		pageDto.setStart(startStr);
		...
		try {
			...
			ExportExcel.reportFM(request,response, pageDto);
