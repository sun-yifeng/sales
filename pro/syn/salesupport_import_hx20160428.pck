create or replace package SALESUPPORT_IMPORT_HX is
  ...
  --销售支持保存审核推荐人，同步数据到核心
  procedure p_single_recommender(i_pk_id       in varchar2, --
                                 o_result_code out varchar2, --
                                 o_result_msg  out varchar2 --
                                 );
...
create or replace package body SALESUPPORT_IMPORT_HX is
    ...
    EXCEPTION
      WHEN OTHERS THEN
        BEGIN
          ROLLBACK;
          sRest    := '-1';
          sMessage := '导入前删除代理人信息失败，错误代码:' || SUBSTR(SQLERRM, 1, 150);
    ...
    EXCEPTION
      WHEN OTHERS THEN
        BEGIN
          ROLLBACK;
          sRest    := '-1';
          sMessage := '导入前删除代理协议信息失败，错误代码:' || SUBSTR(SQLERRM, 1, 150);
          P_WRITE_ERRORLOG(sMessage);
          COMMIT;
          RETURN;
        END;
    ...
    EXCEPTION
      WHEN OTHERS THEN
        BEGIN
          ROLLBACK;
          sRest    := '-1';
          sMessage := '导入前删除代理子协议信息失败，错误代码:' || SUBSTR(SQLERRM, 1, 150);
          P_WRITE_ERRORLOG(sMessage);
          COMMIT;
          RETURN;
        END;
    ...
    EXCEPTION
      WHEN OTHERS THEN
        BEGIN
          ROLLBACK;
          sRest    := '-1';
          sMessage := '导入个人代理信息失败，错误代码:' || SUBSTR(SQLERRM, 1, 150);
          P_WRITE_ERRORLOG(sMessage);
          COMMIT;
          RETURN;
        END;
    ...
    EXCEPTION
      WHEN OTHERS THEN
        BEGIN
          ROLLBACK;
          sRest    := '-1';
          sMessage := '导入机构渠道信息失败，错误代码:' || SUBSTR(SQLERRM, 1, 150);
          P_WRITE_ERRORLOG(sMessage);
          COMMIT;
          RETURN;
        END;
    ...
    EXCEPTION
      WHEN OTHERS THEN
        BEGIN
          ROLLBACK;
          sRest    := '-1';
          sMessage := '更新开户行网点名称失败，错误代码:' || SUBSTR(SQLERRM, 1, 150);
          P_WRITE_ERRORLOG(sMessage);
          COMMIT;
          RETURN;
        END;
    ...
    EXCEPTION
      WHEN OTHERS THEN
        BEGIN
          ROLLBACK;
          sRest    := '-1';
          sMessage := '导入代理协议信息失败，错误代码:' || SUBSTR(SQLERRM, 1, 150);
          P_WRITE_ERRORLOG(sMessage);
          COMMIT;
          RETURN;
        END;
    ...
    EXCEPTION
      WHEN OTHERS THEN
        BEGIN
          ROLLBACK;
          sRest    := '-1';
          sMessage := '导入代理子协议信息失败，错误代码:' || SUBSTR(SQLERRM, 1, 150);
          P_WRITE_ERRORLOG(sMessage);
          COMMIT;
          RETURN;
        END;
  ...
  procedure P_SINGLE_IMPORT(sChaCde in varchar2, sRest out varchar2, sMessage out varchar2) is
    nCount number;
  begin
    dbms_output.put_line('开始同步数据,渠道代码:' || sChaCde || '时间' || to_char(sysdate, 'yyyy-mm-dd hh24:mi:ss'));
    sRest    := '1';
    sMessage := '成功';
  ...
  begin
    dbms_output.put_line('开始同步推荐人:' || i_pk_id);
    begin
      merge into channel_recommender_detail t1
      using channel_reco_detail_orig t2
      on (t1.recommender_id = t2.recommender_id and t1.recommender_id = i_pk_id)
      when matched then
        update
           set t1.recommender_code = t2.recommender_code,
               t1.recommender_name = t2.recommender_name,
               t1.valid_ind        = t2.valid_ind
      when not matched then
        insert
          (recommender_id,
           dept_code,
        ...
        values
          (t2.recommender_id,
           ...
           '1',
           t2.approved_user,
           t2.approved_date) where t2.recommender_id = i_pk_id;
      --更新记录数字
      o_result_msg := '同步推荐人到核心时成功，提交记录数:' || to_char(sql%rowcount);
    exception
      when others then
        begin
          o_result_code := '-1';
          o_result_msg  := '同步推荐人到核心时出错:' || i_pk_id || ',' || substr(sqlerrm, 1, 500);
          rollback;
          return;
        end;
    end;
