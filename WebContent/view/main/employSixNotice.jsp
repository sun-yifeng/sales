...
<title>试用期满六个月的员工转正或淘汰提醒列表</title>
...
$(function(){
	...
	//查询面板
	var searchPanelTitle = "系统提示信息  > 试用期满六个月的员工转正或淘汰提醒列表";
...
var tabHand = [
    [
		...
		{header:"转正情况",name:"managerFlag",width:120,align:'center',
			renderer : function(value, rowData , rowIndex){
				if(value=='0'){
					if(rowData.standFee>=600000)
						return "<span style='color:green'>转正</span>";
					else
						return "<span style='color:red'>淘汰</span>";
						
				}else if(value=='1'){
					if(rowData.standFee>=4000000)
						return "<span style='color:green'>转正</span>";
					else
						return "<span style='color:red'>淘汰</span>";
				}
...
//查询操作
function query() {
	$("#tables").omGrid("setData","<%=_path%>/queryEmploySixNotice.do?"+$("#filterFrm").serialize());
