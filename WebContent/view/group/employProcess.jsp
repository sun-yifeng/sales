...
$(function(){
	...
	$('#businessLine').omCombo({
		...
		onValueChange : function(target, newValue, oldValue, event) {
			...
			var sr_value = $('#saleRank').val();
			if(sr_value!="" && sr_value.indexOf("团队经理") != -1){
				$('#saleRankTwo').omCombo('setData',[]).omCombo({
					dataSource : '<%=_path%>/lawRank/queryManagerRankByLineCode.do?deptCode='+deptCodeTwo+'&lineCode='+newValue+'&param='+Math.random(),
			    });
			}else{
				$('#saleRankTwo').omCombo('setData',[]);
			}
	...
	Util.post("<%=_path%>/salesman/salesmanDetail.do",$("#saleUserFrm").serialize(), function(data) {
		...
		$('#saleRank').omCombo({
			...
			onValueChange:function(target,newValue,oldValue,event){ 
				$.ajax({ 
					url: "<%=_path%>/lawRank/queryRankName.do?rankCode="+newValue,
					type:"post",
					async: false, 
					dataType: "html",
					success: function(data){
						if(data.indexOf("团队经理") == -1){
							$("#customerManagerRank1").hide();
							$("#customerManagerRank2").hide();
						}else{
							...
							$('#saleRankTwo').omCombo('setData',[]).omCombo({
								dataSource : '<%=_path%>/lawRank/queryManagerRankByLineCode.do?deptCode='+deptCodeTwo+'&lineCode='+$("#businessLine").val()+'&param='+Math.random(),
						    });
						}
					}
				});
	...
	$('#groupCode').omCombo({
		...
		onValueChange:function(target,newValue,oldValue,event){ 
			$.ajax({ 
				url: "<%=_path%>/groupMain/getGroupType.do?groupCode=" + newValue,
				type:"post",
				async: false, 
				dataType: "html",
				success: function(data){
					 var str= $.parseJSON(data);
					 var groupType = str.groupType;
					 if(groupType=='2'){
						 $("#entryGroupEle").hide();
						 $("#entryGroupDate").val("2099-12-31");
					 }else{
						 $("#entryGroupEle").show();
					 }
				}
	...
	$("#updateSaleUser").validate({
	   	rules : {
	   		...
	   		areaBrand:"required",
	   		entryGroupDate:{required:true,
	   			valiEntryGroupDate:true}
	   	},
	   	messages : {
	   		groupCode :"销售团队不能为空",
	   		saleRank  : "销售职级不能为空",
	...
	$('#tables').omGrid({
        ...
        colModel : [ {header:"员工代码",name:"salesmanCode",width:150,editor:option=='queryDetail'?null:{editable:false}},
                 	 {header:"员工姓名",name:"salesmanCname",width:150, editor:option=='queryDetail'?null:{editable:false}},
                  	 {header:"身份证号码" , name:"certifyNo",width:200, editor:option=='queryDetail'?null:{editable:false}},
                  	 {header:"当前工号",name:"employCode",width:150, editor:option=='queryDetail'?null:{editable:false}},
                  	 {header:"关联工号" , name:"associateCode",width:150,editor:option=='queryDetail'?null:{rules:[["required",true,"关联工号不能为空"],["maxlength",15,"工号长度不能大于15"]],editable:true}}],
		dataSource : "<%=_path%>/salesmanEmploy/querySalesmanEmploy.do?"+$("#salesmanEmployFrm").serialize()
    });
    //按钮样式
...
function disFn(date){
	 ...
	 nowMiddle.setMonth(nowMiddle.getMonth() - 1);
     if (date < nowMiddle) {
         return false;
     }
}
...
function save(){
	...
	Util.post("<%=_path%>/salesman/updateSalesman.do?entryGroupDate="+$("#entryGroupDate").val(),$("#updateSaleUser").serialize(), function(data) {
