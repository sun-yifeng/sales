...
$(function(){	
	...
	$("#statDate").val($.omCalendar.formatDate(date,'yy-mm-dd'));
	
	var btnum = $("#search-panel").offset().top+$("#search-panel").outerHeight()+85;
...
// 	{header:"本年累计标准保费(元)",name:"groupTotalFee", rowspan:2,width:120,
// 		renderer : function(value, rowData , rowIndex) {
// 			return '<div style="text-align:right;">'+value+'</div>';
// 		}
// 	},
// 	{header:"年化标保时间<br/>进度达成率",name:"groupPlanPercent", rowspan:2,width:80,
// 		renderer : function(value, rowData , rowIndex) {
// 			return '<div style="text-align:right;">'+value+'</div>';
// 		}
// 	},
	{header:"保费收入(元)", colspan:4}],
	[{header:"昨日" , name:"gotFeeYesterday" , width:80,
		renderer : function(value, rowData , rowIndex) {
			return '<div style="text-align:right;">'+value+'</div>';
		}
	},
	{header:"本日" , name:"gotFeeToday" , width:80,
		renderer : function(value, rowData , rowIndex) {
			return '<div style="text-align:right;">'+value+'</div>';
		}
	},
 	{header:"本月" , name:"gotFeeMonth" , width:80,
		renderer : function(value, rowData , rowIndex) {
			return '<div style="text-align:right;">'+value+'</div>';
		}
	},
 	{header:"本年累计" , name:"gotFeeYear" , width:100,
		renderer : function(value, rowData , rowIndex) {
			return '<div style="text-align:right;">'+value+'</div>';
		}
	}]
...
//查询操作
function query(){
	$("#tables").omGrid("setData","<%=_path%>/reportGroupSale/queryReportDayGroupSale.do?"+$("#filterFrm").serialize());
}

//导出Excel
function dataToExcel(){
	window.open("<%=_path%>/reportGroupSale/queryReportDayToExcel.do?"+$("#filterFrm").serialize());
}
		...
		<div id="search-panel">
			<table >
				...
				<tr>
					...
					<td><input type="text"  name="formMap['statDate']"  id="statDate"  value="" class="sele"/></td>
