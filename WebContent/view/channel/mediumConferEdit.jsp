...
$(function(){
    ...
    $('#expireDate').omCalendar();
  	 ...
  	 */
    $.ajax({ 
  		url:"<%=_path%>/common/existRolesInSystemByUserCode.do?roleName=xszcAdmin,headDeptAdminChannelNew,headDeptMediumChanelNew",
  		type:"post",
  		async:true, 
  		dataType:"JSON",
  		success:function(data){
  		  if (data) {
  		     ...
  		     $('#signDate').omCalendar().css({"background":"#FFFFFF"});
  		     $("#conferType").omCombo({readOnly:false}).next(".sele").css({"background-color":"#FFFFFF"});
  		  } else {
	...
	Util.post("<%=_path%>/mediumConfer/queryMediumConfers.do",$("#conferProductFrm").serialize(),function(data) {
			...
			$('#grantDept').omCombo({value : data[0].grantDept});//授权下级机构		
			$('#conferType').omCombo({value : data[0].conferType});//协议类型
	...
	//加载合作机构协议附件信息
	Util.post("<%=_path%>/upload/queryUploadByMainId.do?mainId="+conferCode+"&module=02","",function(data) {
