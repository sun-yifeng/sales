...
$(function(){
	...
	$('#accountTables').omGrid({
         ...
         colModel:[{header:"是否我司员工",name:"isOwnStaff",width:100,align:'center',editor:{rules:["required",true,"此项必选"],type:"omCombo",name:"isOwnStaff",options:isOwnStaffOptions},renderer:isOwnStaffRenderer},
                   ...
                   {header:"出单账号",name:"account",width:150,align:'center',editor:{type:"text",name:"account",editable:false}},
	...
	var salesmanCnameOptions = {
			...
			onValueChange : function(target, newValue, oldValue, event) {
				...
				//根据维护人代码查询用户电话和邮箱
				Util.post(
						"<%=_path%>/salesman/querySalesmanInfoByCode.do?salesmanCode="+newValue,"",
						function(data) {
...
var mediumNodeRule = {
	<%-- nodeCode:{
		required : true,
		remote : "<%=_path%>/mediumNode/queryNodeCode.do",
...
function initValidate(){
	...
	$("#baseInfo").validate({
        ...
        submitHandler : function() {
        	...
        	Util.post(
        		"<%=_path%>/mediumNode/saveMediumNode.do",$("#baseInfo").serialize(), function(data) {
...
//重置
function reset(){
	window.location.href = "mediumNodeAdd.jsp?channelCode="+channelCode+"&mediumCname="+mediumCname+"&deptCode="+deptCode+"&deptName="+deptName;
}
</script>
</head>
<body>
			...
			<form id="baseInfo">
				...
				<table class="fontClass" style="line-height: 25px;">
					...
					<tr>
						...
						<td><input type="text" name="nodeCode" id="nodeCode" /><span style="color:red">*</span></td>
						<td><span class="errorImg"></span><span class="errorMsg"></span></td> 
						-->
	...
	<div>
		<fieldSet>
			...
			<form id="channelMaintainFrm">
				<input type="hidden" name="formMap['nodeCode']" value="" />
