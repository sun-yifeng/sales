...
$(function(){	
    ...
    $("#status").omCombo({
    	dataSource : [{"text":"请选择","value":""},{"text":"草稿","value":"0"},{"text":"已发布","value":"1"},
    	              {"text":"执行结束","value":"3"},{"text":"待发布","value":"5"},{"text":"已终止","value":"9"}] ,
	...
	$('#operationBar').omButtonbar({
    	btns : [
		        ...
		        {label:"终止",
			     ...
			     disabled :  false,
				 icons : {left : Util.appCxtPath + '/core/js/ref/operamasks-ui-2.1/css/default/images/remove.png'},
		 		 onClick:function(){openDetail("noticeValid");}
		        }
			]
    });
	...
	/*
	 * 获取角色权限 修改相应功能
	 * thirdDeptBusiMana 三级机构_业管
	 * thirdDeptMiddle   三级机构_中支总
	 */
	$.ajax({ 
...
var tabHand = [
                ...
                {header:"公告组号",name:"noticGroupId",width:250},
				{header:"公告标题",name:"tiltle",width:150},
				{header:"公告内容",name:"content",width:250},
				{header:"发布时间",name:"publishDate",width:100},
				/* {header:"上次发布时间",name:"periodPre",width:150}, */
				{header:"下次发布时间",name:"periodNext",width:100},
...
//查询操作
function query(){
	$("#tables").omGrid("setData","<%=_path%>/notice/queryNotice.do?action=peroidQuery&"+$("#filterFrm").serialize());
}

//公告新增
function openSave(){
	location.href = "<%=_path%>/notice/noticeRedirect.do?actionName=noticePeriodAdd&soursePage=noticePeriodQuery";
}
...
function openDetail(action){
	try{
	    var sourcePage = "noticePeriodQuery";
		...
		if(cells.length > 1 ){
			...
			if (cells[0].status == '9' && action=='noticeValid' ){
				$.omMessageBox.alert({
	       	        content: "公告已终止,不能再次终止"
	       	    });
				return;
			}if (cells[0].status == '9' && action=='noticeModify' ){
				$.omMessageBox.alert({
	       	        content: "公告已终止,不能修改"
	       	    });
				return;
			}else if (cells[0].status == '1' && action=='noticeDelete' ){
				$.omMessageBox.alert({
	       	        content: '公告正在运行,不能删除'
	       	    });
				return;
			...
			if (cells[0].periodNext != 'undefined' && cells[0].periodNext != undefined
					&& cells[0].periodNext != null && cells[0].periodNext != 'null'
					&& cells[0].periodNext != ''&& action=='noticeValid' ){
				$.omMessageBox.confirm({
			           title:'确认信息',
			           content:'是否要终止该公告?',
			           onClose:function(v){
				            if(v){               
								...
								$("#actionName").val(action);
								$("#soursePage").val(sourcePage);
								document.noticeGrid.action = "<%=_path%>/notice/noticeValid.do";
								document.noticeGrid.submit();
								return;
							}
						}
					...
					return;
			}else if(action=='noticeValid'){
				$.omMessageBox.alert({
	       	        content: '请选择"下次发布时间"不为空的公告操作'
	       	    });
				return;
			}
		...
		<div id="search-panel">
			<table>
				...
				<tr>
					...
					<td style="padding-left:15px" align="right" ><span class="label">公告组号：</span></td>
					<td><input type="text" name="formMap['noticGroupId']" id="noticGroupId" style="width:158px;"/></td>
					<td></td>
