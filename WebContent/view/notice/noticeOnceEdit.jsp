...
/* 	var businessLine = noticeInfoJson.businessLine.split(",");
	...
	if(noticeInfoJson.publishRule=="1"){
		$('#publishDate').omCalendar({editable : false,disabled : true});
	}
	$("#publishDate").val(noticeInfoJson.publishDate);
...
function showUploadFile(){
	...
	//getImg("#jdSummary-upload-download",{method:"queryUploadByMainId",module:"08",mainId:noticeInfoJson.noticGroupId,operateCode:'none'});
...
function noticeSave(status){
	...
	var publishDate = $("#publishDate").val();
     if($("#filterFrm").validate().form()){
			Util.post("<%=_path%>/notice/noticeUpdate.do?"+$("#filterFrm").serialize(),{publishDate:publishDate},
					function(data){
						$.omMessageBox.alert({
			                type:'success',
			                title:'成功',
			...
			<legend class="fontClass" style="margin-left: 40px;">基本信息</legend>
				...
				<table class="fontClass" style="">
					<tr>
		                ...
		                <td  nowrap><input readonly="readonly" class="disabled:true" name="noticId" id="noticId"/>
