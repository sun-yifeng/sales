...
$(function(){
	...
	$("#feedbackContent").val(noticeJson.feedbackContent);
	$("#feedbackId").val(noticeJson.feedbackId);
	$("#deptCode").val(noticeJson.deptCode);
	...
	$("#button-argue").omButton({icons : {left : '<%=_path%>/images/op-edit.png'},width:50});
...
function showUploadFile()
{
	...
	getImg("#feedBack-upload-download",{method:"queryTimingUploadFeedback",module:"10",mainId:noticeInfoJson.noticId,feedbackId:noticeInfoJson.feedbackId,deptCode:noticeJson.deptCode,operateCode:'none'});
}
 ...
 */
function initPage(){
		$(":input:not(#processContent)").attr("readonly","true");
...
function dealSave(){	
	...
	{
		Util.post("<%=_path%>/notice/dealSave.do?"+$("#filterFrm").serialize(),{noticId:$("#noticId").val(),feedbackId:$("#feedbackId").val(),updatedUser:noticeJson.updatedUser},
				function(data){
					$.omMessageBox.alert({
		                type:'success',
		                title:'成功',
		                content:'<font style="font:bold">处理成功!</font>',
		                onClose:function(value){
		                    $("#filterFrm")[0].reset();
		                    window.location.href = "<%=_path%>/view/notice/noticeDeal.jsp";
		                }
		            });
				});
	}
...
function dealArgue(){	
	...
	{
		Util.post("<%=_path%>/notice/dealArgue.do?"+$("#filterFrm").serialize(),{noticId:$("#noticId").val(),feedbackId:$("#feedbackId").val(),updatedUser:noticeJson.updatedUser},
				function(data){
					$.omMessageBox.alert({
		                type:'success',
		                title:'成功',
		                content:'<font style="font:bold">驳回成功!</font>',
		                onClose:function(value){
		                    $("#filterFrm")[0].reset();
		                    window.location.href = "<%=_path%>/view/notice/noticeDeal.jsp";
		                }
...
<!-- 		                <td colspan="6"><div name="receiveDept" id="receiveDept" readOnly = "true"></div> -->
<!-- 		                </td> -->
...
<!-- 			<div id="feedBack-upload-download"  style="width: 100%;"></div> -->
			...
			<div id="feedbackTab">
					<div id="addFeedBackDiv">
						<div >
							<table>
								<tr>
					                <td width="120px" align="right"><span class="color_red">*</span><span class="label">反馈内容：</span></td>
					                <td colspan="5"><textarea rows="2" cols="69" class="omRequired" name="feedbackContent" id="feedbackContent" readOnly = "true"></textarea>
					                </td>
								</tr>
							</table>
						</div>
			...
			<div id="feedbackTab">
					<div id="addFeedBackDiv">
						<div >
							<table>
								<tr>									
					                <td width="120px" align="right"><span class="color_red">*</span><span class="label">处理内容：</span></td>
					                <td colspan="5"><textarea rows="2" cols="69" class="omRequired" name="processContent" id="processContent"  maxlength="200"></textarea></td>
					                <td width="30px"><span class="errorImg"></span><span class="errorMsg" ></span></td>
								</tr>
							</table>
						</div>
			...
			<table style="width: 100%">
				<tr>
					...
					<a class="om-button" id="button-save" onclick="dealSave()">处理</a>
					<a class="om-button" id="button-argue" onclick="dealArgue()">驳回</a>
