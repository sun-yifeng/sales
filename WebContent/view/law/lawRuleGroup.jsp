...
var factorSysOptions = {
	dataSource:"<%=_path%>/lawDefine/queryRules.do?itemType=5",
	onValueChange:function(target, newValue, oldValue, event) {
		$.ajax({
	   		 url: "<%=_path%>/lawDefine/queryRules.do?itemType=5", 
	   			type:"post",
	   			async: false, 
	   			dataType:"json",
	   			success:function(data){
...
var factorImpOptions = {
	dataSource:"<%=_path%>/lawDefine/queryRules.do?itemType=6",
	onValueChange:function(target, newValue, oldValue, event) {
		$.ajax({
	   		 url: "<%=_path%>/lawDefine/queryRules.do?itemType=6", 
	   			type:"post",
	   			async: false, 
	   			dataType:"json",
	   			success:function(data){
...
var factorExaOptions = {
	dataSource:"<%=_path%>/lawDefine/queryRules.do?itemType=7",
	onValueChange:function(target, newValue, oldValue, event) {
		$.ajax({
	   		 url: "<%=_path%>/lawDefine/queryRules.do?itemType=7", 
	   			type:"post",
	   			async: false, 
	   			dataType:"json",
	   			success:function(data){
...
var indexCodeOptions = {
	dataSource:"<%=_path%>/lawDefine/queryRules.do?itemType=8",
	onValueChange:function(target, newValue, oldValue, event) {
		$.ajax({
	   		 url: "<%=_path%>/lawDefine/queryRules.do?itemType=8", 
	   			type:"post",
	   			async: false, 
	   			dataType:"json",
	   			success:function(data){
...
var checkCodeOptions = {
	dataSource:"<%=_path%>/lawDefine/queryRulesE.do?itemType=9",
    editable: false
};
//考核操作
...
$(function() {
	...
	$('#tablesImport').omGrid({
	    ...
	    onSuccess:function(data,testStatus,XMLHttpRequest,event){
	    	//$(this).omGrid({height : 80 + data.rows.length * 24});
	    	//$('#tablesFactor').omGrid({height : 80 + data.rows.length * 24});
	    	//$('#tablesExamine').omGrid({height : 80 + data.rows.length * 24});
	    }
	});
   //考核因素
...
function buttonContr(){
	$.ajax({ 
		...
		success: function(data){
			...
			//系统因素
			if(statusStr.groupSysSwitch == '0'){
				$("#sysButton").omButton("disable");
			}
			//导入因素
			if(statusStr.groupImpSwitch == '0'){
				$("#impButton").omButton("disable");
			}
			//考核因素
			if(statusStr.groupCheckSwitch == '0'){
				$("#checkButton").omButton("disable");
			}
			//计算公式
			if(statusStr.groupExpSwitch == '0'){
				$("#expButton").omButton("disable");
			}
			//考核规则
			if(statusStr.groupRegSwitch == '0'){
