...
function getTableHead(){
	//表头
	var tabHand = [
		{header:"保单号",name:"policyNumber",width:150,align:'left'},
		{header:"被保险人",name:"insuredPerson",width:150,align:'left'},
		{header:"业务员工号",name:"businessEmployeeNumber",width:150,align:'left'},
		{header:"业务员姓名",name:"businessName",width:150,align:'left'},
		{header:"批单号",name:"batchNumber",width:150,align:'left'},
		{header:"月份",name:"dataMonth",width:150,align:'left'}
	];
...
//加载导入数值类型
function getLawImpTypeList(versionId){
	$.ajax({ 
		url: "<%=_path%>/lawImpValue/getLawDefineList",
		data:{itemType:"1",versionId:versionId},
		type:"post",
		async: false,
		dataType: "json",
...
function downloadXlsModel(){
	window.location.href = "<%=_path %>/ExportExcel/ElectricPolicyTemplate.xls";
	return true;
}
...
//查询操作
function query() {
	$("#tables").omGrid("setData","<%=_path%>/lawImpValue/queryLawElectricPolicy.do?"+$("#filterFrm").serialize());
...
<body>
	<form id="filterFrm">
	        <input type="hidden" name="formMap['impType']"  value="2" >
			...
			<table>
				<tr>
					...
					<td style="padding-left:15px" align="right"><span class="label">业务员工号：</span></td>
					<td><input class="sele" name="formMap['businessEmployeeNumber']" id="businessEmployeeNumber" /></td>
	...
	<div id="impXlsArea" style="display:none">
	    ...
	    <form action="<%=_path %>/lawImpValue/lawElectricPolicyUpload" id="impXlsForm" method="post" class="easyWebForm" target="submitFrame" enctype="multipart/form-data">
			<input type="hidden" name="impType" id="impType" value="2" >
			<input type="hidden" name="versionId"  value="${param.versionId }" >
			<table>
				<tr>
				  <td style="text-align:center" colspan="2">
				          导入说明：导入的批单号请如实填写<br/>
				          如没有批单号码请留空，不可填入'0'或'-'<br/>
				     <br/>
