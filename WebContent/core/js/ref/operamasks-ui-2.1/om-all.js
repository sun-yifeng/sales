		...
		_showLoadingMessage: function(){
			...
			if($loadMsg.length === 0){
				if("default" === options.loadingMessage){
					$("<div class='om-panel-loadingMessage'><div class='valignMiddle'><div class='loadingImg'></div></div></div>")
...
;(function($) {
    $.omWidget('om.omGrid', {
        ...
        _buildTableHead:function(){
            ...
            //渲染序号列
            if(op.showIndex){
                var cell=$('<th></th>').attr({axis:'indexCol',align:'center'}).addClass('indexCol').append($('<div class="indexheader" style="text-align:center;width:45px;">序号</div>'));
                tr.append(cell);
                indexWidth=25;
            }
            //渲染checkbox列
        ...
        _populate : function() { // get latest data
            ...
            // gjwu 2013-12-11 增加取数据的方式，可以直接传递相应格式的json,或者传递function，返回值作为json数据
            var dd = op.dataSource;
            if ( $.isFunction( dd ) ) { // 调用方法，使用返回值为作数据
            	dd = dd.apply( window, [ nowPage, limit ] );
            }
            if ( $.isPlainObject( dd ) ) { // 直接传数据
            	if (typeof(op.onSuccess) == 'function') {
                    self._trigger("onSuccess",null,dd,"success",null);
                }
                self._addData(dd);
                for(var i=0 , len=op._onRefreshCallbacks.length; i<len; i++){
                	op._onRefreshCallbacks[i].call(self);
                }
                self._trigger("onRefresh",null,nowPage,dd.rows);
                loadMask.hide();
                self.loading = false;
            } else {
        ...
        //获取分页的数据json；
        getPagationInfo:function(){
        	return {pageNo:this.pageData.nowPage,pageSize:this.options.limit};
        },
        destroy:function(){
        	var $el = this.element;
 ...
 * operamasks-ui omAjaxSubmit 2.1
 *
 * Copyright 2011, AUTHORS.txt (http://ui.operamasks.org/about)
 * Dual licensed under the MIT or LGPL Version 2 licenses.
 * http://ui.operamasks.org/license
 ...
 * operamasks-ui omBorderLayout 2.1
 *
 * Copyright 2011, AUTHORS.txt (http://ui.operamasks.org/about)
 * Dual licensed under the MIT or LGPL Version 2 licenses.
 * http://ui.operamasks.org/license
        ...
        _showDropList:function(){
            ...
            if (!options.listAutoWidth) {
            	...
            	if($.browser.msie&&($.browser.version == "7.0")&&!$.support.style){
            		dropListContainer.width(dropList.show().outerWidth()+50);
            	}else{
            		dropListContainer.width(dropList.outerWidth()+50);
            	}
            }
 ...
 * operamasks-ui omFileUpload 2.1
 *
 * Copyright 2011, AUTHORS.txt (http://ui.operamasks.org/about)
 * Dual licensed under the MIT or LGPL Version 2 licenses.
 * http://ui.operamasks.org/license
...
//	                $grid = $elem.closest('.om-grid'),
	            ...
	            for(var i=0,row=cm.length; i<row; i++){
	            	...
	            	if(i == 0){//行号和多选框标题列永远在第一行
	            		if(op.showIndex){
	            			content.push("<th class='indexCol data-header-"+row+"' align='center' axis='indexCol' rowspan="+row+"><div class='indexheader' style='text-align:center;width:45px;'>序号</div></th>");
	            		}
	            		if(!op.singleSelect){
...
//			$editRow,
		...
		if($editView.length == 0){
			$editView = $("<div class='grid-edit-view'><div class='body-wrapper'><div class='grid-edit-row'><form class='grid-edit-form'></form></div>"
					+"<div class='gird-edit-btn'>" 
					+"<input type='button' class='ok om-grid-btn' value='确定'/>" 
					+"<input type='button' class='cancel om-grid-btn' value='取消'/></div></div></div>")
        ...
        value:function(newValue){
            if(arguments.length==0){ //getValue
                ...
                if($.isArray(newValue)){
                    ...
                    //FIXME baiyunfei this._buildList();
                    this._buildList2();
                }
            }
        },
...
(function( $, undefined ) {
	...
	var close = function(messageBox, mask, handler, value){
	    ...
	    // gjwu 2013-12-05 修改回调方法的 this对象为messageBox
	    handler ? handler.apply(messageBox, [value]) : jQuery.noop();
	    messageBox.remove();
	    mask.remove();
	};
    var _show = function(config){
        ...
        var messageBox = $(tmpl).appendTo(document.body).css('z-index', 1500).position({
            ...
            if (event.keyCode && event.keyCode === $.om.keyCode.ESCAPE) {
            	...
            	// gjwu 2013-12-05 按esc键关闭时，也调用onClose方法
        ...
        var closeBut = messageBox.find('span.om-messageBox-title').html(config.title).next().hover(function(){
        	...
        	//close(messageBox, mask, null, false);
        	// gjwu 2013-12-05 按标题栏的叉关闭键关闭时，也调用onClose方法
            close(messageBox, mask, onClose, false);
            return false;
 ...
 * operamasks-ui omNumberField 2.1
 *
 * Copyright 2011, AUTHORS.txt (http://ui.operamasks.org/about)
 * Dual licensed under the MIT or LGPL Version 2 licenses.
 * http://ui.operamasks.org/license
 ...
 * operamasks-ui omProgressbar 2.1
 *
 ...
 * operamasks-ui omSuggestion 2.1
 *
 * Copyright 2011, AUTHORS.txt (http://ui.operamasks.org/about)
 * Dual licensed under the MIT or LGPL Version 2 licenses.
 * http://ui.operamasks.org/license
