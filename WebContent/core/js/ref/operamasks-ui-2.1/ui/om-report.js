/*
 * $Id: om-report.js,v 1.164 2012/06/29 09:20:32 chentianzhen Exp $
 * operamasks-ui omReport 2.1
 *
...
/**
	 ...
	 */
;(function($) {
    $.omWidget('om.omReport', {
        options:/** @lends omReport#*/{
            //外观
            /**
             ...
             * @type Boolean
             * @example
             * $('.selector').omReport({autoFit : true});
             */
            autoFit:false,
            /**
             * 是否在最左边显示序号列。
             ...
             * @type Boolean
             * @example
             * $('.selector').omReport({showIndex : false});
             */
            showIndex:true,
            //数据源
            /**
             ...
             * //该文件必须返回一段具有特定格式（格式可参考文档的“预览”页签的说明）的JSON数据，omReport拿到该数据即可用来填充表格。
             * $('.selector').omReport({url:'griddata.json'});
             */
            dataSource:false,
             /**
             ...
             * $('.selector').omReport({url:'griddata.json',extraData:{googType:1,localtion:'beijing'} });
             */
            extraData:{},
            /**
             ...
             * @default 'GET'
             * @example
             * $('.selector').omReport({method : 'POST'});
             */
            method:'GET',
            /**
             * 正在取数时显示在分页条上的提示。
             * @name omReport#loadingMsg
             * @type String
             * @default '正在加载数据，请稍候...'
             * @example
             * $('.selector').omReport({loadingMsg : '取数中...'});
             */
            //loadingMsg:$.om.lang.omReport.loadingMsg,
            /**
             * 取数完成后但是后台没有返回任何数据时显示在分页条上的提示。
             * @name omReport#emptyMsg
             * @type String
             * @default '没有数据'
             * @example
             * $('.selector').omReport({emptyMsg : 'No data!'});
             */
            //emptyMsg:$.om.lang.omReport.emptyMsg,
            /**
             * 取数发生错误时显示在分页条上的提示。
             * @name omReport#errorMsg
             * @type String
             * @default '取数出错'
             * @example
             * $('.selector').omReport({emptyMsg : '应用异常，请与网站管理员联系!'});
             */
            //errorMsg:$.om.lang.omReport.errorMsg,
            /**
             ...
             * $('.selector').omReport({preProcess : function(data){
             *          var temp;
             ...
             * $('.selector').omReport({rowClasses : ['class1','class2','class2']});
             * 
             * //示例2：满分的行加上样式fullMarks，不及格的行加上样式flunk，其它行使用默认样式。
             * $('.selector').omReport({rowClasses : function(rowIndex,rowData){
             ...
             * @default true
             * @example
             * $('.selector').omReport({singleSelect : false});
             */
            singleSelect:true,
            /**
             * 设置组件的标题
             ...
             * @default ''
             * @example
             * $('.selector').omReport({title : 'Data Grid'});
             */
            title: '',
            // report
            tableName: '',
             ...
             *              }
             *          });
             *          $('.selector').omReport('setSelections',rows);
             *      }
             *  });
             */
            onRefresh:function(nowPage,pageRecords,event){},
        ...
        _create:function(){
            var options=this.options,el=this.element.show() // show if hidden
                ...
                .empty()
                .append('<tbody></tbody>');
            el.wrap('<div class="om-report om-widget om-widget-content"><div class="bDiv" style="width:auto"></div></div>').closest('.om-report');
        ...
        /**
				 * 改变组件的大小。
				 * @name omReport#resize
				 * @function
				 * @param position (1)可以为Object,格式如{width:500,height:250} <br/>
				 *                 (2)只有一个参数表示width,有两个参数时依次表示width,height
				 * @example
				 * $(".selector").omReport("resize" , 500);<br />//把宽度改为500像素。
				 * $(".selector").omReport("resize" , 500 , 250);<br />//把宽度改为500像素，高度改为250像素。
				 * $(".selector").omReport("resize" , {height:300});<br />//把高度改为300像素。
				 * $(".selector").omReport("resize" , {width:'fit',height:'fit'});<br />//把宽度和高度同时改为自适应父容器大小。
				 * 
				 */
        resize : function(position){
        	var self = this,
        		ops = this.options,
        		$grid = this.element.closest('.om-report');
        ...
        _fixHeaderWidth:function(autoExpandColIndex , allColsWidth){
        	var $grid = this.element.closest('.om-report'),
        		$thead = $('thead',this.hDiv),
        ...
        _buildPagingToolBar:function(){
            ...
            pDiv.show().html('<div class="pDiv2">'+
            	...
            	'</div>');
            var pageText = $.om.lang._get(op,"omReport","pageText").replace(/{totalPage}/, '<span>1</span>').replace(/{index}/, '<input type="text" size="4" value="1" />');
        ...
        _changePage : function(ctype) { // change page
            ...
            switch (ctype) {
                ...
                case 'last':
                    ...
                    break;
                case 'input':
                    var nv = parseInt($('.pControl input', el.closest('.om-report')).val());
                    if (isNaN(nv)) {
                        nv = nowPage;
                    }
                    if (nv < 1) {
                ...
                default:
                    if (/\d/.test(ctype)) {
                        ...
                        $('.pControl input', el.closest('.om-report')).val(nv);
                        newPage = nv;
                    }
            }
            if (newPage == nowPage) {
        ...
        _populate : function() { // get latest data
            ...
            //具备加载数据的前提条件了，准备加载
            this.loading = true;
            pageStat.html($.om.lang._get(op,"omReport","loadingMsg"));
            loadMask.show();
            ...
            $.ajax({
                ...
                error : function(XMLHttpRequest, textStatus, errorThrown) {
                    pageStat.html($.om.lang._get(op,"omReport","errorMsg")).css('color','red');
                    try {
        ...
        _buildPager:function(){
            ...
            var pager=this.pager,
                ...
                pageStat='';
            if(data.total===0){
                pageStat=$.om.lang._get(op,"omReport","emptyMsg");
            }else{
                pageStat = $.om.lang._get(op,"omReport","pageStat").replace(/{from}/, from).replace(/{to}/, to).replace(/{total}/, data.total);
            }
        ...
        _renderDatas:function(){
            ...
            $.each(rows,function(i, rowData) {
                ...
                bodyContent.push("<tr _grid_row_id="+(self._guid++)+" class='om-report-row " + rowCls + "'>");
        ...
        _bindSelectAndClickEnvent:function(){
            ...
            if(!this.options.singleSelect){ //可以多选
                ...
                this.tbody.delegate('tr.om-report-row','click',function(event){
                ...
                //行双击
                this.tbody.delegate('tr.om-report-row','dblclick',function(event){
                    var row=$(this),index=self._getRowIndex(row);
                    if(row.hasClass('om-state-highlight')){ //已选择
                        //do nothing
                    }else{
        ...
        _makeColsResizable:function(){
            var self=this,
                ...
                grid=self.element.closest('.om-report'),
		...
		_getTrs:function(){
			return this.tbody.find("tr.om-report-row:not([_delete]=true)");		
		},
        //public methods
        /**
         ...
         * @name omReport#setData
         * @function
         * @param url 新的数据源url
         * @returns jQuery对象
         * @example
         *  //使用新的url来取数，设置完后会立即开始刷新表格数据。
         *  $('.selector').omReport('setData', 'newgriddata.json');
         */
         ...
         * @name omReport#refresh
         * @function
         * @returns jQuery对象
         * @example
         * //根据当前grid数据模型中的数据，重新刷新grid
         * $('.selector').omReport('refresh');//注意refresh没有传入参数
         * 
         */
        refresh:function(){
            ...
            this.loading = true;
            var op=this.options;
			$('.pPageStat', this.pager).html($.om.lang._get(op,"omReport","loadingMsg"));
            this.loadMask.show();
            this._buildPager();
            this._renderDatas();
         ...
         * @name omReport#reload
         * @function
         * @param page 要转到的页，参数为空表示刷新当前页。如果参数不是数字或者小于1则自动修正为1，如果参数大于总页数则自动修正为总页数。
         * @returns jQuery对象
         * @example
         * $('.selector').omReport('reload');//刷新当前页
         * $('.selector').omReport('reload',3);//转到第3页
         * 
         */
         ...
         * @name omReport#setSelections
         * @function
         * @param indexes 序号（第一行是0第二行是1）数组。
         * @returns jQuery对象
         * @example
         * //选择表格中第二行、第三行、第五行
         * $('.selector').omReport('setSelections',[1,2,4]);
         * 
         */
         ...
         * @name omReport#getSelections
         * @function
         * @param needRecords 参数为true时返回的不是行序号数组而是行记录数组。参数为空或不是true时返回行序号数组。
         * @returns jQuery对象
         * @example
         * var selectedIndexed = $('.selector').omReport('getSelections');
         * var selectedRecords = $('.selector').omReport('getSelections',true);
         * 
         */
        ...
        destroy:function(){
        	var $el = this.element;
        	$el.closest('.om-report').after($el).remove();
        }
    });
